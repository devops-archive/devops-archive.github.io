...
```
mkdir -vp ~/projects/devops-archive/http-delay
```

```
touch ~/projects/devops-archive/http-delay/curl-stats.txt

```
Lets create the contents for the statistics file.

Paste the below content to: `~/projects/devops-archive/http-delay/curl-stats.txt`
...
```
curl -w "@${HOME}/projects/devops-archive/http-delay/curl-stats.txt" -o /dev/null -s "http://wordpress.com/"
```
